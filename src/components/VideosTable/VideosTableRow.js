...
const VideosTableRow = ({ videoId, title, description }) => {
    ...
    // handler function to handle edit video
    const editVideoHandler = async () => {
        await triggerGetVideo(videoId, false);
    }

    // handler function to handle delete video
    const deleteVideoHandler = () => {
        deleteVideo(videoId);
    }

    // rendering videos table row component here
    return (
        <tr>
            <td className='table-td'>{title}</td>
            <td className='table-td'>{description.length > 50 ? description.substring(0, 50) + '...' : description}</td>
            <td className='table-td flex gap-x-2 justify-center'>
                <button onClick={deleteVideoHandler} className='disabled:opacity-80 disabled:cursor-not-allowed' disabled={isDltLoading}>
                ...
                <button onClick={editVideoHandler} className='disabled:opacity-80 disabled:cursor-not-allowed' disabled={isLoading}>
                    <svg fill='none' viewBox='0 0 24 24' strokeWidth='1.5' stroke='currentColor'
                        className='w-6 h-6 hover:text-secondary cursor-pointer transition-all'>
                        <path strokeLinecap='round' strokeLinejoin='round'
