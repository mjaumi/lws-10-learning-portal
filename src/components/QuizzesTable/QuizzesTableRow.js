...
const QuizzesTableRow = ({ quizId, question, videoTitle }) => {
    ...
    // handler function to handle quiz delete
    const deleteQuizHandler = () => {
        deleteQuiz(quizId);
    }

    // handler function to handle quiz edit
    const editQuizHandler = () => {
        triggerGetQuiz(quizId, false);
    }

    // rendering quizzes table row component here 
    return (
        <tr>
            <td className='table-td'>Quiz {quizId} - {question}</td>
            <td className='table-td'>{videoTitle}</td>
            <td className='table-td flex gap-x-2 justify-center'>
                <button onClick={deleteQuizHandler} className='disabled:opacity-80 disabled:cursor-not-allowed' disabled={isLoading}>
                ...
                <button onClick={editQuizHandler} className='disabled:opacity-80 disabled:cursor-not-allowed' disabled={isEditLoading}>
                    <svg fill='none' viewBox='0 0 24 24' strokeWidth='1.5' stroke='currentColor'
