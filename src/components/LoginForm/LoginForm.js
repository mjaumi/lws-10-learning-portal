...
const LoginForm = () => {
    ...
    return (
        <form onSubmit={loginHandler} className='mt-8 space-y-6'>
            ...
            <div>
                <button type='submit'
                    className='btn group relative w-full flex justify-center !py-2 !px-4 border border-transparent text-sm font-medium rounded-md text-white bg-violet-600 hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500 disabled:opacity-80 disabled:cursor-not-allowed disabled:loading capitalize' disabled={isLoading}>
                    Sign in
                </button>
            </div>
        </form>
